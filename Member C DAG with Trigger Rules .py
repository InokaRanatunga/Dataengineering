...
# Automatically finds your home directory (e.g. /home/core)
...
default_args = {
    "owner": "Group_Member_C",
    "depends_on_past": False,
    "start_date": days_ago(1), # Ready to run immediately
    "email_on_failure": False,
    "retries": 0,  # 0 retries ensures validation failure stops the DAG instantly
    "retry_delay": timedelta(minutes=2),
}

dag = DAG(
    "weather_etl_final_project",
...
def create_monthly_aggregates(**kwargs):
    ...
    # Create 'month' string column (e.g. 2006-04)
...
# Task 6: Validation (Member C)
def validate_data(**kwargs):
    """
    Member C Task: Check data quality.
    Raises ValueError (stops pipeline) if constraints are violated.
    """
    daily_file = kwargs["ti"].xcom_pull(key="daily_file", task_ids="create_daily_table")
    ...
    # 2. Check Temperature Range (-80 to +80)
...
def load_data_to_sqlite(**kwargs):
    """
    Member C Task: Load validated data to SQLite.
    Only runs if Validation succeeds.
    """
    daily_file = kwargs["ti"].xcom_pull(key="daily_file", task_ids="create_daily_table")
    monthly_file = kwargs["ti"].xcom_pull(key="monthly_file", task_ids="create_monthly_table")
...
load_task = PythonOperator(
    ...
    dag=dag,
    # TRIGGER RULE: Ensures this only runs if Validation (Upstream) succeeds.
...
clean_task >> [daily_task, monthly_task] >> validate_task >> load_task